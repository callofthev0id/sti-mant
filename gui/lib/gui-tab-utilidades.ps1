...
# listado|aviso. delicado = punto naranja (7.4). preset = entra en Preset STI (7.6, excluye delicados).
#
...
# 'sentidoOn' que dice si "feature ON" coincide con "lo que STI recomienda" (acordToggle):
...
#                          STI recomienda apagarlo; el toggle ON refleja el estado REAL, no la accion).
function Get-StiUtilCatalogo {
  @(
...
function Get-StiUtilCategorias {
  @(
...
function Get-StiUtilPorCategoria {
  param([string]$Categoria)
  @(Get-StiUtilCatalogo | Where-Object { $_.categoria -eq $Categoria })
}
...
# FIXTURE (tests) o del detector real (Get-StiUtilDeteccionReal). NO corre nada: solo da forma al dato
# para la UI. Devuelve estado + display + clase de color.
function Resolve-StiUtilEstado {
...
# $Estados es el array de salidas de Resolve-StiUtilEstado.
function Get-StiUtilResumen {
...
# Ids del Preset STI (spec 7.6): los marcados preset=$true, excluyendo delicados (decision conservadora).
function Get-StiUtilPreset {
  @(Get-StiUtilCatalogo | Where-Object { $_.preset -and -not $_.delicado } | ForEach-Object { $_.id })
}
...
function Format-StiPopoverItem {
  param([hashtable]$Raw)
...
# Resolve-StiUtilEstado. Para items toggle: estado='aplicado' cuando la feature esta presente (toggle
...
function Get-StiServicioEstado {
  param([string]$Name)
  try {
...
function Test-StiUtil {
  ...
  try {
    switch ($Item.id) {
      # --- Tweaks (registro) ---
      'ctxclasico' {
        $v = Get-StiRegValor 'HKCU:\Software\Classes\CLSID\{86ca1aa0-34aa-4e8b-a509-50c905bae2a2}\InprocServer32' '(default)'
        if ($null -ne $v) { return @{ estado='aplicado'; dato='clasico' } } else { return @{ estado='disponible'; dato='Win11' } }
      }
      'telemetria' {
        $v = Get-StiRegValor 'HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection' 'AllowTelemetry'
        if ($null -ne $v -and [int]$v -eq 0) { return @{ estado='disponible'; dato='apagada' } }
        return @{ estado='aplicado'; dato='activa' }
      }
      'darkmode' {
        $v = Get-StiRegValor 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Themes\Personalize' 'AppsUseLightTheme'
        if ($null -ne $v -and [int]$v -eq 0) { return $on } else { return $off }
      }
      'visorfotos' {
        $v = Get-StiRegValor 'HKLM:\SOFTWARE\Microsoft\Windows Photo Viewer\Capabilities\FileAssociations' '.jpg'
        if ($null -ne $v) { return @{ estado='aplicado'; dato='asociado' } } else { return $off }
      }
      'extocultos' {
        $ext = Get-StiRegValor 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced' 'HideFileExt'
        $hid = Get-StiRegValor 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced' 'Hidden'
        if (($null -ne $ext -and [int]$ext -eq 0) -and ($null -ne $hid -and [int]$hid -eq 1)) { return $on }
        return $off
      }
      'faststartup' {
        $v = Get-StiRegValor 'HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Power' 'HiberbootEnabled'
      ...
      'esteequipo' {
        $v = Get-StiRegValor 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel' '{20D04FE0-3AEA-1069-A2D8-08002B30309D}'
        if ($null -ne $v -and [int]$v -eq 0) { return @{ estado='aplicado'; dato='visible' } } else { return $off }
      }
      'bingcortana' {
        $v = Get-StiRegValor 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Search' 'BingSearchEnabled'
        if ($null -ne $v -and [int]$v -eq 0) { return @{ estado='disponible'; dato='quitado' } }
        return @{ estado='aplicado'; dato='activo' }
      }
      # --- Debloat (appx / registro) ---
      'copilot' {
        $v = Get-StiRegValor 'HKCU:\Software\Policies\Microsoft\Windows\WindowsCopilot' 'TurnOffWindowsCopilot'
      ...
      'edgedebloat' {
        $v = Get-StiRegValor 'HKLM:\SOFTWARE\Policies\Microsoft\Edge' 'HubsSidebarEnabled'
        if ($null -ne $v -and [int]$v -eq 0) { return @{ estado='disponible'; dato='off' } }
        return @{ estado='aplicado'; dato='activo' }
      }
      # --- Servicios ---
      'diagtrack' {
        $e = Get-StiServicioEstado 'DiagTrack'
...
function Get-StiUtilDeteccionReal {
  $out = @{}
  foreach ($item in (Get-StiUtilCatalogo)) {
    $out[$item.id] = Test-StiUtil -Item $item
  }
  $out
}
...
# appx/tareas). Tolerantes a fallo: ante excepcion devuelven array vacio. Get-StiUtilListadosReal
# arma el hashtable id -> array que consume Update-UtilidadesPanel/New-StiUtilPopover.

# Apps de inicio: claves Run (HKLM/HKCU) + carpetas Startup. Impacto heuristico por nombre.
function Get-StiListadoAppsInicio {
  $out = @()
  $runKeys = @(
...
function Get-StiListadoServiciosInnec {
...
function Get-StiListadoBloatware {
...
function Get-StiListadoAppsOem {
  $out = @()
  try {
...
function Get-StiListadoTareasTele {
  $out = @()
...
function Get-StiUtilListadosReal {
  @{
    appsinicio = @(Get-StiListadoAppsInicio)
    servinnec  = @(Get-StiListadoServiciosInnec)
    bloatware  = @(Get-StiListadoBloatware)
    appsoem    = @(Get-StiListadoAppsOem)
    tareastele = @(Get-StiListadoTareasTele)
  }
}
...
# (best-effort) y SIEMPRE se audita cada accion (scan/apply/undo/safety) via Write-StiAudit
...
function Get-StiUtilLogPath {
  if (Get-Command Get-StiAuditTextPath -ErrorAction SilentlyContinue) { return (Get-StiAuditTextPath) }
  'C:\zback\sti-utilidades-log.txt'
}

# Wrapper de compatibilidad: las acciones de utilidades auditan via Write-StiAudit. Mantiene la
...
function Write-StiUtilLog {
  ...
  try {
    ...
    $cat = ''
    try { $cat = (Get-StiUtilCatalogo | Where-Object id -eq $Id | Select-Object -First 1).categoria } catch {}
    $label = ''
    try { $label = (Get-StiUtilCatalogo | Where-Object id -eq $Id | Select-Object -First 1).nombre } catch {}
    Write-StiAudit -Accion $accion -UtilId $Id -UtilLabel $label -Categoria $cat `
...
function New-StiUtilCheckpoint {
  param([string]$Descripcion = 'STI Utilidades - antes de aplicar')
  try {
    Checkpoint-Computer -Description $Descripcion -RestorePointType 'MODIFY_SETTINGS' -ErrorAction Stop
    Write-StiUtilLog -Id '_checkpoint' -Modo 'safety' -Despues $Descripcion -Resultado 'ok' | Out-Null
    return $true
  } catch {
    Write-StiUtilLog -Id '_checkpoint' -Modo 'safety' -Resultado "fallo: $($_.Exception.Message)" | Out-Null
    return $false
  }
}

# Helper: escribe un valor de registro creando la clave si no existe. Lo usan las acciones de tweak.
function Set-StiRegValor {
...
# Invoke-StiUtilAccion (no finge exito) y la UI (muestra el aviso).
function Get-StiUtilRequiereAdmin {
...
# ¿El item id requiere admin? (helper de consulta sobre Get-StiUtilRequiereAdmin).
function Test-StiUtilRequiereAdmin {
  param([string]$Id)
  return ((Get-StiUtilRequiereAdmin) -contains $Id)
}
...
# ante click real via Invoke-StiUtilAccion -> el batch (Invoke-StiUtilBatch) agrega checkpoint+log.
# Para los tweaks/servicios reversibles, 'aplicar' deja el estado que STI recomienda y 'revertir'
# vuelve al estado por defecto de Windows; el detector read-only refleja el cambio al re-escanear.
function Get-StiUtilAcciones {
  @{
    ...
    reindex   = @{ aplicar = 'Stop-Service WSearch -Force -ErrorAction SilentlyContinue; Set-StiRegValor "HKLM:\SOFTWARE\Microsoft\Windows Search" "SetupCompletedSuccessfully" 0; Start-Service WSearch -ErrorAction SilentlyContinue' }
    ...
    ctxclasico  = @{
      aplicar = 'Set-StiRegValor "HKCU:\Software\Classes\CLSID\{86ca1aa0-34aa-4e8b-a509-50c905bae2a2}\InprocServer32" "(default)" "" "String"'
    ...
    telemetria  = @{
      aplicar = 'Set-StiRegValor "HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection" "AllowTelemetry" 0'
      revertir = 'Remove-StiRegValor "HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection" "AllowTelemetry"' }
    darkmode    = @{
      aplicar = 'Set-StiRegValor "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Themes\Personalize" "AppsUseLightTheme" 0; Set-StiRegValor "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Themes\Personalize" "SystemUsesLightTheme" 0'
      revertir = 'Set-StiRegValor "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Themes\Personalize" "AppsUseLightTheme" 1; Set-StiRegValor "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Themes\Personalize" "SystemUsesLightTheme" 1' }
    visorfotos  = @{
      aplicar = 'foreach ($ext in @(".jpg",".jpeg",".png",".bmp",".gif")) { Set-StiRegValor "HKLM:\SOFTWARE\Microsoft\Windows Photo Viewer\Capabilities\FileAssociations" $ext "PhotoViewer.FileAssoc.Tiff" "String" }'
      revertir = 'foreach ($ext in @(".jpg",".jpeg",".png",".bmp",".gif")) { Remove-StiRegValor "HKLM:\SOFTWARE\Microsoft\Windows Photo Viewer\Capabilities\FileAssociations" $ext }' }
    extocultos  = @{
      aplicar = 'Set-StiRegValor "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" "HideFileExt" 0; Set-StiRegValor "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" "Hidden" 1'
      revertir = 'Set-StiRegValor "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" "HideFileExt" 1; Set-StiRegValor "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced" "Hidden" 2' }
    faststartup = @{
      aplicar = 'Set-StiRegValor "HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Power" "HiberbootEnabled" 0'
      revertir = 'Set-StiRegValor "HKLM:\SYSTEM\CurrentControlSet\Control\Session Manager\Power" "HiberbootEnabled" 1' }
    energia     = @{
    ...
    esteequipo  = @{
      aplicar = 'Set-StiRegValor "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel" "{20D04FE0-3AEA-1069-A2D8-08002B30309D}" 0'
      revertir = 'Set-StiRegValor "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel" "{20D04FE0-3AEA-1069-A2D8-08002B30309D}" 1' }
    bingcortana = @{
      aplicar = 'Set-StiRegValor "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Search" "BingSearchEnabled" 0; Set-StiRegValor "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Search" "CortanaConsent" 0'
      revertir = 'Set-StiRegValor "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Search" "BingSearchEnabled" 1' }
    ...
    copilot   = @{ aplicar = 'Set-StiRegValor "HKCU:\Software\Policies\Microsoft\Windows\WindowsCopilot" "TurnOffWindowsCopilot" 1' }
    ...
    edgedebloat = @{ aplicar = 'Set-StiRegValor "HKLM:\SOFTWARE\Policies\Microsoft\Edge" "HubsSidebarEnabled" 0' }
  }
}
...
function Get-StiUtilAccionesConsola {
...
# Solo para los ids de Get-StiUtilAccionesConsola; para el resto devuelve $null (se streamea).
function Get-StiUtilComandoConsola {
  param([string]$Id)
  $cmd = switch ($Id) {
    'sfc'    { 'sfc /scannow' }
    ...
    'wsreset'{ 'wsreset.exe' }
    'repwu'  { 'net stop wuauserv & net stop bits & net stop cryptsvc & for /f "tokens=2 delims==" %T in (''wmic os get localdatetime /value ^| find "="'') do set STIWUTS=%T & ren "%WINDIR%\SoftwareDistribution" "SoftwareDistribution.bak_%STIWUTS:~0,14%" & net start wuauserv & net start bits & net start cryptsvc' }
    default  { $null }
  }
  if (-not $cmd) { return $null }
  # /k deja la consola abierta; el title ayuda al tecnico a identificarla.
  "/k title STI - $Id & echo === STI Utilidades: $Id === & echo. & $cmd & echo. & echo === Termino. Podes cerrar esta ventana. ==="
}
...
function Resolve-StiUtilEjecucion {
  param([string]$Fase = 'listo', [string]$Detalle = '')
  switch ($Fase) {
    'ejecutando' { @{ fase='ejecutando'; etiqueta='Ejecutando...'; habilitado=$false; progreso=$true;  clase='e-run'; color='#e8b94f'; texto = if ($Detalle) { $Detalle } else { 'Ejecutando...' } } }
    'ok'         { @{ fase='ok';         etiqueta='Aplicar';       habilitado=$true;  progreso=$false; clase='e-ok';  color='#43C961'; texto = if ($Detalle) { $Detalle } else { 'Listo' } } }
    'error'      { @{ fase='error';      etiqueta='Reintentar';    habilitado=$true;  progreso=$false; clase='e-err'; color='#ff7b78'; texto = if ($Detalle) { $Detalle } else { 'Error' } } }
    default      { @{ fase='listo';      etiqueta='Aplicar';       habilitado=$true;  progreso=$false; clase='e-idle';color='#9fdcc0'; texto = if ($Detalle) { $Detalle } else { '' } } }
  }
}
...
# bloquea el hilo STA. Devuelve un job { ps; handle; queue; rs } consumible por Receive-StiUtilStream.
# El comando se ejecuta con [scriptblock]::Create del texto real (Get-StiUtilAcciones). Best-effort:
# captura excepciones y las encola como lineas 'ERROR: ...'. El log de auditoria lo hace el caller.
function Start-StiUtilAccionStream {
  ...
  # actual (incluidas las de esta lib: Set-StiRegValor, etc). Asi el comando corre SIN depender de
...
function Receive-StiUtilStream {
  param($Job)
  $lineas = @()
  $out = ''
...
function Get-StiUtilValorActual {
  param([string]$Id)
  try {
    $det = Test-StiUtil -Item (Get-StiUtilCatalogo | Where-Object { $_.id -eq $Id } | Select-Object -First 1)
...
# El checkpoint del batch lo arma Invoke-StiUtilBatch (una vez por batch, no por util). Devuelve texto.
function Invoke-StiUtilAccion {
  ...
  if ((Test-StiUtilRequiereAdmin -Id $Id) -and -not (Test-StiUtilElevado)) {
    if (-not $SinLog) { Write-StiUtilLog -Id $Id -Modo $Modo -Resultado 'error: sin privilegios' | Out-Null }
    return "requiere admin: $Id ($Modo)"
  }
  $antes = if ($SinLog) { '' } else { Get-StiUtilValorActual -Id $Id }
  try {
    $sb = [scriptblock]::Create($txt)
    & $sb | Out-Null
    $despues = if ($SinLog) { '' } else { Get-StiUtilValorActual -Id $Id }
    if (-not $SinLog) { Write-StiUtilLog -Id $Id -Modo $Modo -Antes $antes -Despues $despues -Resultado 'ok' | Out-Null }
    return "ok: $Id ($Modo)"
  } catch {
    if (-not $SinLog) { Write-StiUtilLog -Id $Id -Modo $Modo -Antes $antes -Resultado "error: $($_.Exception.Message)" | Out-Null }
...
# servicios/debloat, no limpieza/reparaciones) y log por cada util via Invoke-StiUtilAccion.
...
function Invoke-StiUtilBatch {
  param([hashtable]$Pendientes, [scriptblock]$Confirmador)
  $resultados = @()
  $catById = @{}
  foreach ($it in (Get-StiUtilCatalogo)) { $catById[$it.id] = $it }
  ...
  $checkpoint = $false
  if ($persistente) { $checkpoint = New-StiUtilCheckpoint }
  ...
  if ($checkpointFailed -and $noReversibles.Count -gt 0) {
    ...
    if ($Confirmador) {
      ...
      try {
        ...
        $okSeguir = ([System.Windows.MessageBox]::Show($msg, 'STI - sin checkpoint', 'YesNo', 'Warning') -eq 'Yes')
  ...
  foreach ($id in @($Pendientes.Keys)) {
    if ($omitirNoRev -and ($noReversibles -contains $id)) {
      ...
      Write-StiUtilLog -Id $id -Modo $Pendientes[$id] -Resultado 'omitido: sin checkpoint (no reversible)' | Out-Null
      continue
    }
    $resultados += (Invoke-StiUtilAccion -Id $id -Modo $Pendientes[$id])
  }
...
# Format-StiPopoverItem. Cada fila lleva checkbox (seleccion manual) + ubicacion + badge de impacto.
function New-StiUtilPopover {
  param($Anchor, [string]$Titulo, $Items, [string]$AccionId = '', [scriptblock]$OnConfirm)
  $norm = @()
  foreach ($r in @($Items)) { if ($r -is [hashtable]) { $norm += (Format-StiPopoverItem $r) } }
  ...
  $card.Background = '#0e1a15'; $card.BorderBrush = '#43C961'; $card.BorderThickness = 1
  ...
  $h.Foreground = '#6fb597'; $h.FontFamily = 'Space Grotesk, Segoe UI'; $h.FontWeight = 'Bold'
  ...
  if (-not $norm.Count) {
    ...
    $empty.Text = 'Sin elementos detectados.'; $empty.Foreground = '#9cc3b2'; $empty.FontSize = 11
    ...
    foreach ($it in $norm) {
      ...
      $nm.Text = $it.nombre; $nm.Foreground = '#eaf2ec'; $nm.FontSize = 11; $nm.TextTrimming = 'CharacterEllipsis'
      [void]$info.Children.Add($nm)
      if ($it.ubicacion) {
        $ub = New-Object System.Windows.Controls.TextBlock
        $ub.Text = $it.ubicacion; $ub.Foreground = '#7fb89e'; $ub.FontFamily = 'DM Mono, Consolas'
      ...
      $impHex = switch ($it.clase) { 'i-hi' { '#ff7b78' } 'i-md' { '#e8b94f' } default { '#5fd47f' } }
      $bg.Background = '#13241c'; $bg.BorderBrush = $impHex; $bg.BorderThickness = 1
  ...
  $resumen.Text = "$($norm.Count) elementos · selección manual"; $resumen.Foreground = '#9cc3b2'
  ...
  $ok.Background = '#43C961'; $ok.Foreground = '#053028'
...
# ventana y delega en Invoke-StiUtilAccionUI. Tolerante: si no hay config (test sin wiring) igual
# corre con ScriptDir vacio y sin re-escaneo.
function Invoke-StiUtilAccionDesdeClick {
  param($Window, [string]$Id, $Boton)
  $item = $null
  try { $item = Get-StiUtilCatalogo | Where-Object id -eq $Id | Select-Object -First 1 } catch {}
  $label = if ($item) { [string]$item.nombre } else { '' }
  $consola = (Get-StiUtilAccionesConsola) -contains $Id
  ...
  try {
    $resp = [System.Windows.MessageBox]::Show($aviso, 'STI', 'YesNo', 'Warning')
    if ($resp -ne 'Yes') { return }
  } catch {}
  $cfg = Get-StiUtilCfg -Window $Window
  $sd = [string]$cfg['ScriptDir']
  $onDone = $cfg['OnRescan']
  Invoke-StiUtilAccionUI -Window $Window -Id $Id -Modo 'aplicar' -Boton $Boton -OnDone $onDone -ScriptDir $sd
}
...
function Get-StiUtilPendientes {
  param($Window)
...
# registra (FIX seguridad #7) para que el batch sepa que elementos eligio el tecnico. Get-StiUtilCatalogo
...
function Set-StiUtilSeleccion {
  param($Window, [string]$Id, $Marcados)
  $sel = Get-StiUtilSeleccion -Window $Window
  $arr = @($Marcados)
  if ($arr.Count -gt 0) { $sel[$Id] = $arr } else { [void]$sel.Remove($Id) }
  Update-StiUtilPendientesUI -Window $Window
  $arr.Count
}

# Refresca el contador "N cambios pendientes de aplicar" en el footer.
function Update-StiUtilPendientesUI {
  param($Window)
  $pend = Get-StiUtilPendientes -Window $Window
  $n = @($pend.Keys).Count
...
function Set-StiToggleVisual {
  param($Toggle, $Knob, [bool]$On)
  $verde = '#43C961'
  $Toggle.Background  = if ($On) { $verde } else { '#23382c' }
  $Toggle.BorderBrush = if ($On) { $verde } else { '#34543f' }
  $Knob.Background    = if ($On) { '#053028' } else { '#6a8a7b' }
...
function New-PanelUtilidadesXaml {
  $cats = Get-StiUtilCategorias
  $bloques = foreach ($c in $cats) {
    $items = "Items" + (Get-Culture).TextInfo.ToTitleCase($c.id)
    @"
        <Grid Margin="0,11,0,6">
          <TextBlock Foreground="{StaticResource StiTenue}" FontWeight="700" FontSize="11" Text="$([System.Security.SecurityElement]::Escape($c.titulo.ToUpper()))" HorizontalAlignment="Left"/>
          <TextBlock Foreground="{StaticResource StiTexto3}" FontFamily="DM Mono, Consolas" FontSize="9" Text="$([System.Security.SecurityElement]::Escape($c.nat))" HorizontalAlignment="Right"/>
        </Grid>
...
<ScrollViewer x:Name="PanelUtilidades" Visibility="Collapsed" VerticalScrollBarVisibility="Auto">
  <StackPanel>
    <Border Background="#0c1f17" BorderBrush="{StaticResource StiBordeSutil}" BorderThickness="0,0,0,1" Padding="0,9" Margin="0,0,0,4">
      <StackPanel>
        <StackPanel Orientation="Horizontal">
          <TextBlock x:Name="TxtUtilResumen" VerticalAlignment="Center" Foreground="{StaticResource StiTexto2}" FontSize="11" Text="Estado detectado al abrir"/>
          <Button x:Name="BtnUtilReescanear" Content="Re-escanear" Margin="10,0,6,0" Padding="11,5"/>
          <Button x:Name="BtnUtilPreset" Style="{StaticResource StiBtnPrimary}" Content="Preset STI" Padding="11,5"/>
        </StackPanel>
        <Grid Margin="0,7,0,0">
          <ProgressBar x:Name="ProgUtil" Height="4" Background="#12241b" BorderThickness="0" Foreground="#43C961" IsIndeterminate="False" Visibility="Collapsed"/>
        </Grid>
      </StackPanel>
    </Border>
$($bloques -join "`n")
    <StackPanel Orientation="Horizontal" Margin="0,8,0,0">
      <TextBlock Foreground="{StaticResource StiTexto3}" FontSize="9" Text="punto naranja = revisar antes de aplicar" Margin="0,0,14,0"/>
      <TextBlock Foreground="{StaticResource StiTexto3}" FontSize="9" Text="toggle verde = feature activa ahora (se puede revertir)" Margin="0,0,14,0"/>
      <TextBlock Foreground="{StaticResource StiTexto3}" FontSize="9" Text="historial de cambios en el registro de auditoria" Margin="0,0,14,0"/>
      <TextBlock Foreground="{StaticResource StiTexto3}" FontSize="9" Text="elegir... = popover de seleccion"/>
    </StackPanel>
    <Border Background="#0b2018" BorderBrush="{StaticResource StiBordeSutil}" BorderThickness="0,1,0,0" Padding="0,11" Margin="0,8,0,0">
      <StackPanel Orientation="Horizontal">
        <TextBlock x:Name="TxtUtilPendientes" VerticalAlignment="Center" Foreground="{StaticResource StiTexto2}" FontSize="11" Text="0 cambios pendientes de aplicar"/>
        <Button x:Name="BtnUtilRevertir" Content="Revertir aplicadas" Margin="12,0,6,0" Padding="13,8"/>
        <Button x:Name="BtnUtilAplicar" Style="{StaticResource StiBtnPrimary}" Content="Aplicar cambios" Padding="16,9"/>
      </StackPanel>
    </Border>
    <Border Background="#0a1812" BorderBrush="{StaticResource StiBordeSutil}" BorderThickness="1" CornerRadius="6" Padding="10,8" Margin="0,10,0,4">
      <StackPanel>
        <StackPanel Orientation="Horizontal" Margin="0,0,0,6">
          <TextBlock Foreground="{StaticResource StiTenue}" FontWeight="700" FontSize="11" Text="REGISTRO DE AUDITORIA" VerticalAlignment="Center"/>
          <TextBlock x:Name="TxtUtilLogInfo" Foreground="{StaticResource StiTexto3}" FontFamily="DM Mono, Consolas" FontSize="9" Margin="10,0,0,0" VerticalAlignment="Center" Text="Event Log + JSON + texto"/>
        ...
        <Border Background="#06120d" BorderBrush="#1d3326" BorderThickness="1" CornerRadius="4" Padding="6,4">
...
function Update-UtilidadesPanel {
  ...
  $naranja = '#E07820'
  $sOn = '#5fd47f'; $sOff = '#8fb0a0'; $sNa = '#5a7567'
  $pend = Get-StiUtilPendientes -Window $Window
  $resueltos = @()
  foreach ($cat in (Get-StiUtilCategorias)) {
    $ctrlName = 'Items' + (Get-Culture).TextInfo.ToTitleCase($cat.id)
    $ctrl = $Window.FindName($ctrlName)
    if (-not $ctrl) { continue }
    $ctrl.Items.Clear()
    foreach ($item in (Get-StiUtilPorCategoria $cat.id)) {
      $det = if ($Estados.ContainsKey($item.id)) { $Estados[$item.id] } else { @{} }
      $est = Resolve-StiUtilEstado -Item $item -Deteccion $det
      $resueltos += $est

      $row = New-Object System.Windows.Controls.Border
      $row.Background = '#13241c'; $row.BorderBrush = '#243d31'; $row.BorderThickness = 1
      ...
      $nm = New-Object System.Windows.Controls.TextBlock
      $nm.Text = $item.nombre; $nm.Foreground = '#e2eee7'; $nm.FontSize = 11.5
      ...
      switch ($item.tipo) {
        'listado' {
          ...
          $lk.Content = 'elegir...'; $lk.Foreground = '#9fdcc0'; $lk.Background = '#16331f'
          $lk.BorderBrush = '#2e5a47'; $lk.FontSize = 10; $lk.Padding = '8,3'; $lk.Cursor = 'Hand'
          $listado = if ($Listados.ContainsKey($item.id)) { $Listados[$item.id] } else { @() }
          $winL = $Window
          $onConf = { param($accId, $marcados) Set-StiUtilSeleccion -Window $winL -Id $accId -Marcados $marcados | Out-Null }.GetNewClosure()
          $pop = New-StiUtilPopover -Anchor $lk -Titulo $item.nombre -Items $listado -AccionId $item.id -OnConfirm $onConf
          if (@($listado).Count -gt 0) { $lk.BorderBrush = '#43C961' }
        ...
        'aviso' {
          ...
          $nr.Text = 'no reversible'; $nr.Foreground = '#e8b94f'; $nr.FontFamily = 'DM Mono, Consolas'
          ...
          $ap.Content = 'Aplicar'; $ap.Foreground = '#9fdcc0'; $ap.Background = '#16331f'
          $ap.BorderBrush = '#2e5a47'; $ap.FontSize = 10; $ap.Padding = '8,3'; $ap.Cursor = 'Hand'
          $idA = $item.id; $winA = $Window
          $ap.Add_Click({ param($s,$e) Invoke-StiUtilAccionDesdeClick -Window $winA -Id $idA -Boton $s }.GetNewClosure())
        ...
        default {
          ...
          $knob.Margin = '1,0,1,0'
          $tg.Child = $knob
          Set-StiToggleVisual -Toggle $tg -Knob $knob -On $on
          ...
          $tg.AddHandler(
            ...
            [System.Windows.Input.MouseButtonEventHandler]{ param($s,$e)
              $t = $s.Tag
              $t.cur = -not $t.cur
              Set-StiToggleVisual -Toggle $s -Knob $knobRef -On $t.cur
              $stRef.Text = if ($t.cur) { 'si' } else { 'no' }
              $stRef.Foreground = if ($t.cur) { '#5fd47f' } else { '#8fb0a0' }
              $p = Get-StiUtilPendientes -Window $winT
              if ($t.cur -eq $t.base) {
                [void]$p.Remove($t.id)
              } else {
                $p[$t.id] = if ($t.cur) { 'aplicar' } else { 'revertir' }
              }
              Update-StiUtilPendientesUI -Window $winT
            }.GetNewClosure()
          )
  ...
  $resumen = Get-StiUtilResumen -Estados $resueltos
...
# Lee via Get-StiAuditRecent (mas recientes primero) y pinta una fila por entrada. Se llama al abrir
...
function Update-StiUtilLogPanel {
  param($Window, [int]$Count = 25, [object[]]$Records)
  $ctrl = $Window.FindName('ItemsUtilLog')
  if (-not $ctrl) { return }
  if ($null -eq $Records) {
    $Records = @()
    try { if (Get-Command Get-StiAuditRecent -ErrorAction SilentlyContinue) { $Records = @(Get-StiAuditRecent -Count $Count) } } catch {}
  }
  $ctrl.Items.Clear()
  if (-not @($Records).Count) {
    $tb = New-Object System.Windows.Controls.TextBlock
    $tb.Text = 'Sin registros de auditoria todavia.'; $tb.Foreground = '#5a7567'
    $tb.FontFamily = 'DM Mono, Consolas'; $tb.FontSize = 9.5; $tb.Margin = '2,1'
    [void]$ctrl.Items.Add($tb)
    return
  }
  foreach ($r in @($Records)) {
    $line = Format-StiAuditPanelLine -Record $r
    $tb = New-Object System.Windows.Controls.TextBlock
    $tb.Text = $line; $tb.FontFamily = 'DM Mono, Consolas'; $tb.FontSize = 9.5; $tb.Margin = '2,1'
    $err = ([string]$r.resultado -match '^(?i)error')
    $tb.Foreground = if ($err) { '#ff7b78' } elseif (([string]$r.accion) -eq 'apply') { '#9fe7bd' } else { '#9cc3b2' }
    [void]$ctrl.Items.Add($tb)
  }
}
...
function Add-StiUtilLogLinea {
  ...
  $tb.Foreground = if ($Texto -match '^(?i)error|ERROR:') { '#ff7b78' } elseif ($Texto -match '^>') { '#43C961' } else { '#9cc3b2' }
  [void]$ctrl.Items.Add($tb)
}
...
#   - consola larga (Get-StiUtilAccionesConsola): abre una CONSOLA VISIBLE (cmd /k) para que el
...
#   - resto: corre async (Start-StiUtilAccionStream) y STREAMEA stdout al panel de log en vivo; el
...
function Invoke-StiUtilAccionUI {
  ...
  if ((Get-StiUtilAccionesConsola) -contains $Id) {
    $cmdArgs = Get-StiUtilComandoConsola -Id $Id
    Add-StiUtilLogLinea -Window $Window -Texto "> $label [$Id]: abriendo consola visible..."
    $resultado = 'ok'
    try { Start-Process -FilePath 'cmd.exe' -ArgumentList $cmdArgs -WindowStyle Normal | Out-Null }
    catch { $resultado = "error: $($_.Exception.Message)"; Add-StiUtilLogLinea -Window $Window -Texto "ERROR: $($_.Exception.Message)" }
    try { Write-StiAudit -Accion 'apply' -UtilId $Id -UtilLabel $label -Categoria $cat -Mensaje 'lanzado en consola visible' -Resultado $resultado | Out-Null } catch {}
    Add-StiUtilLogLinea -Window $Window -Texto "> $label [$Id]: consola lanzada (segui el progreso en la ventana)"
    if ($OnDone) { & $OnDone }
    return
  }

  # --- Via STREAMING al panel para acciones cortas ---
  $estIni = Resolve-StiUtilEjecucion -Fase 'ejecutando'
  ...
  Add-StiUtilLogLinea -Window $Window -Texto "> $label [$Id]: ejecutando..."

  $antes = Get-StiUtilValorActual -Id $Id
  $job = Start-StiUtilAccionStream -Id $Id -Modo $Modo -ScriptDir $ScriptDir
  ...
  $timer.Add_Tick({
    ...
    foreach ($l in $r.lineas) { Add-StiUtilLogLinea -Window $winR -Texto $l }
    if (-not $r.done) {
      $ticks.Value++
      ...
      $estTo = Resolve-StiUtilEjecucion -Fase 'error' -Detalle 'Timeout'
      if ($btnR) { $btnR.IsEnabled = $estTo.habilitado; $btnR.Content = $estTo.etiqueta }
      Add-StiUtilLogLinea -Window $winR -Texto "ERROR: $labelR [$idR]: timeout tras $([int]($maxTicks*120/1000))s, se corto la ejecucion"
      try { Write-StiAudit -Accion 'apply' -UtilId $idR -UtilLabel $labelR -Categoria $catR -EstadoAnterior $antesR -Resultado 'error: timeout' | Out-Null } catch {}
      if ($onDoneR) { & $onDoneR }
      return
    }
    $timer.Stop()
    ...
    $est = Resolve-StiUtilEjecucion -Fase $fase
    if ($btnR) { $btnR.IsEnabled = $est.habilitado; $btnR.Content = $est.etiqueta }
    $despues = Get-StiUtilValorActual -Id $idR
    $res = if ($fase -eq 'ok') { 'ok' } else { 'error: la accion fallo (ver salida)' }
    try { Write-StiAudit -Accion 'apply' -UtilId $idR -UtilLabel $labelR -Categoria $catR -EstadoAnterior $antesR -EstadoNuevo $despues -Resultado $res | Out-Null } catch {}
    Add-StiUtilLogLinea -Window $winR -Texto "> $labelR [$idR]: $(if ($fase -eq 'ok') { 'completado' } else { 'con errores' })"
    if ($onDoneR) { & $onDoneR }
  }.GetNewClosure())
  $timer.Start()
}
